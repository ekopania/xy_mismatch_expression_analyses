...
names(group_to_cross)<-c("CCPP_LZ", "CCPP_RS", "CCPPLY_LZ", "CCPPLY_RS", "LLPP_LZ", "LLPP_RS", "LLPPLY_LZ", "LLPPLY_RS", "PPLLPY_LZ", "PPLLPY_RS", "PPLL_LZ", "PPLL_RS", "WWLLPY_LZ", "WWLLPY_RS", "WWLL_LZ", "WWLL_RS")
...
geneids<-rep(rownames(group_rpkm_x), 4)
rpkm_x_df<-as.data.frame(cbind(rpkm_value, geno, mismatch, geneids))
...
mismatch<-grepl("CCPPLY|WWLLPY|LLPP_|PPLL_",geno)
geneids<-rep(rownames(group_rpkm_y, 4))
rpkm_y_df<-as.data.frame(cbind(rpkm_value, geno, mismatch, geneids))
...
print("Plotting...")
p_x<-ggplot(rpkm_x_df, aes(x=geno, y=log(rpkm_value))) + geom_violin(aes(fill=mismatch)) + geom_boxplot(width=0.1)
...
p_y<-ggplot(rpkm_y_df, aes(x=geno, y=log(rpkm_value))) + geom_violin(aes(fill=mismatch)) + geom_boxplot(width=0.1)
...
mismatch<-grepl("CCPPLY|WWLLPY|LLPP_|PPLL_",geno)
rpkm_x_df_proco<-as.data.frame(cbind(rpkm_value, geno, mismatch))
...
mismatch<-grepl("CCPPLY|WWLLPY|LLPP_|PPLL_",geno)
rpkm_y_df_proco<-as.data.frame(cbind(rpkm_value, geno, mismatch))
...
print("Plotting...")
p_x<-ggplot(rpkm_x_df_proco, aes(x=geno, y=log(rpkm_value))) + geom_violin(aes(fill=mismatch)) + geom_boxplot(width=0.1)
p_x<-p_x + labs(title="FPKM on the X chromosome by cross type - protein coding genes only", x="Cross type", y="log(Group FPKM)")
p_x<-p_x + theme(axis.text.y = element_text(size=20))
p_x<-p_x + theme_minimal() #+ ylim(0,100)
p_x<-p_x + scale_fill_manual(values=c("darkolivegreen","darkorchid4"))

p_y<-ggplot(rpkm_y_df_proco, aes(x=geno, y=log(rpkm_value))) + geom_violin(aes(fill=mismatch)) + geom_boxplot(width=0.1)
...
#Plot expression changes per-gene
print("Plotting gene expression changes per-gene...")

#Get expression change pattern
exp_patterns<-c()
for(i in 1:nrow(rpkm_x_df)){
	this_gene<-rpkm_x_df$geneids[i]
	cross1_exp<-rpkm_x_df$rpkm_value[intersect(which(rpkm_x_df$geneids==this_gene), which(rpkm_x_df$geno==colnames(group_rpkm)[1]))]
	cross2_exp<-rpkm_x_df$rpkm_value[intersect(which(rpkm_x_df$geneids==this_gene), which(rpkm_x_df$geno==colnames(group_rpkm)[2]))]
	cross3_exp<-rpkm_x_df$rpkm_value[intersect(which(rpkm_x_df$geneids==this_gene), which(rpkm_x_df$geno==colnames(group_rpkm)[3]))]
	cross4_exp<-rpkm_x_df$rpkm_value[intersect(which(rpkm_x_df$geneids==this_gene), which(rpkm_x_df$geno==colnames(group_rpkm)[4]))]
        #print(cross1_exp)
	#print(cross2_exp)
	if( (cross1_exp > 1.5*cross2_exp) && (cross3_exp > 1.5*cross4_exp) ){
		exp_patterns<-c(exp_patterns, "oneGTtwo,threeGTfour")
	} else if( (cross1_exp > 1.5*cross2_exp) && (1.5*cross3_exp < cross4_exp) ){
		exp_patterns<-c(exp_patterns, "oneGTtwo,threeLTfour")
	} else if( (1.5*cross1_exp < cross2_exp) && (1.5*cross3_exp < cross4_exp) ){
		exp_patterns<-c(exp_patterns, "oneLTtwo,threeLTfour")
	} else if( (1.5*cross1_exp < cross2_exp) && (cross3_exp > 1.5*cross4_exp) ){
		exp_patterns<-c(exp_patterns, "oneLTtwo,threeGTfour")
	} else{
		exp_patterns<-c(exp_patterns, "other")
	}
}
new_x_df<-as.data.frame(cbind(rpkm_x_df, exp_patterns))
print("On X:")
print(paste("Higher in CCPP than CCPPLY and higher in WWLL than WWLLPY:", length(which(new_x_df$exp_patterns=="oneGTtwo,threeLTfour"))))
print(paste("Higher in CCPPLY than CCPP and higher in WWLLPY than WWLL:", length(which(new_x_df$exp_patterns=="oneLTtwo,threeGTfour"))))
print(paste("Higher in CCPP than CCPPLY and higher in WWLLPY than WWLL:", length(which(new_x_df$exp_patterns=="oneGTtwo,threeGTfour"))))
print(paste("Higher in CCPPLY than CCPP and higher in WWLL than WWLLPY:", length(which(new_x_df$exp_patterns=="oneLTtwo,threeLTfour"))))

exp_patterns<-c()
for(i in 1:nrow(rpkm_y_df)){
        this_gene<-rpkm_y_df$geneids[i]
        cross1_exp<-rpkm_y_df$rpkm_value[intersect(which(rpkm_y_df$geneids==this_gene), which(rpkm_y_df$geno==colnames(group_rpkm)[1]))]
        cross2_exp<-rpkm_y_df$rpkm_value[intersect(which(rpkm_y_df$geneids==this_gene), which(rpkm_y_df$geno==colnames(group_rpkm)[2]))]
	cross3_exp<-rpkm_y_df$rpkm_value[intersect(which(rpkm_y_df$geneids==this_gene), which(rpkm_y_df$geno==colnames(group_rpkm)[3]))]
	cross4_exp<-rpkm_y_df$rpkm_value[intersect(which(rpkm_y_df$geneids==this_gene), which(rpkm_y_df$geno==colnames(group_rpkm)[4]))]
	#print(cross1_exp)
	#print(cross2_exp)
	if( (cross1_exp > 1.5*cross2_exp) && (cross3_exp > 1.5*cross4_exp) ){
                exp_patterns<-c(exp_patterns, "oneGTtwo,threeGTfour")
...
new_y_df<-as.data.frame(cbind(rpkm_y_df, exp_patterns))
print("On Y:")
print(paste("Higher in CCPP than CCPPLY and higher in WWLL than WWLLPY:", length(which(new_y_df$exp_patterns=="oneGTtwo,threeLTfour"))))
print(paste("Higher in CCPPLY than CCPP and higher in WWLLPY than WWLL:", length(which(new_y_df$exp_patterns=="oneLTtwo,threeGTfour"))))
print(paste("Higher in CCPP than CCPPLY and higher in WWLLPY than WWLL:", length(which(new_y_df$exp_patterns=="oneGTtwo,threeGTfour"))))
print(paste("Higher in CCPPLY than CCPP and higher in WWLL than WWLLPY:", length(which(new_y_df$exp_patterns=="oneLTtwo,threeLTfour"))))

p_x<-ggplot(new_x_df, aes(x=geno, y=log(rpkm_value), group=geneids, col=exp_patterns)) + geom_line() + geom_point()
p_x<-p_x + labs(title="FPKM on the X chromosome by cross type", x="Cross type", y="log(Group FPKM)")
p_x<-p_x + theme(axis.text.y = element_text(size=20))
p_x<-p_x + theme_minimal() #+ ylim(0,100)
#p_x<-p_x + scale_fill_manual(values=c("darkolivegreen","darkorchid4"))

p_y<-ggplot(new_y_df, aes(x=geno, y=log(rpkm_value), group=geneids, col=exp_patterns)) + geom_line() + geom_point()
p_y<-p_y + labs(title="FPKM on the Y chromosome by cross type", x="Cross type", y="log(Group FPKM)")
p_y<-p_y + theme(ayis.text.y = element_text(size=20))
p_y<-p_y + theme_minimal() #+ ylim(0,100)
#p_y<-p_y + scale_fill_manual(values=c("darkolivegreen","darkorchid4"))
plots.list<-list(p_x,p_y)
plots<-marrangeGrob(plots.list,nrow=1,ncol=1)
ggsave(paste("fpkm_by_sexChr.points",dataset,cell_type,"pdf",sep="."), plots, width = 11, height = 8.5, units = "in")
