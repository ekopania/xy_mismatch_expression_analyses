...
exp1_musX<-read.table(paste0("topDEgenes.lrt.DE.CCPP_",cell_type,"vsCCPPLY_",cell_type,".removeIntrogressed.bed"), header=TRUE)
exp1_musY<-read.table(paste0("topDEgenes.lrt.DE.CCPP_",cell_type,"vsWWLLPY_",cell_type,".removeIntrogressed.bed"), header=TRUE)
exp1_domX<-read.table(paste0("topDEgenes.lrt.DE.WWLL_",cell_type,"vsWWLLPY_",cell_type,".removeIntrogressed.bed"), header=TRUE)
exp1_domY<-read.table(paste0("topDEgenes.lrt.DE.CCPPLY_",cell_type,"vsWWLL_",cell_type,".removeIntrogressed.bed"), header=TRUE)

exp2_musX<-read.table(paste0("topDEgenes.lrt.DE.PPLL_",cell_type,"vsPPLLPY_",cell_type,".removeIntrogressed.bed"), header=TRUE)
exp2_musY<-read.table(paste0("topDEgenes.lrt.DE.LLPP_",cell_type,"vsPPLLPY_",cell_type,".removeIntrogressed.bed"), header=TRUE)
exp2_domX<-read.table(paste0("topDEgenes.lrt.DE.LLPP_",cell_type,"vsLLPPLY_",cell_type,".removeIntrogressed.bed"), header=TRUE)
exp2_domY<-read.table(paste0("topDEgenes.lrt.DE.LLPPLY_",cell_type,"vsPPLL_",cell_type,".removeIntrogressed.bed"), header=TRUE)
...
#Plot
print("Plotting...")
pdf(paste("upset_plot",cell_type,"removeIntrogressed.pdf", sep="."), onefile=TRUE)
...
pdf(paste("upset_plot.noDirectionality",cell_type,"removeIntrogressed.musXonly.pdf", sep="."), onefile=TRUE)
...
#If there are no DE genes in this category, list will have a character(0), which upset can't process, so remove them here
listInput_x_X_no0<-listInput_x_X[lapply(listInput_x_X, length)>0]
upset(fromList(listInput_x_X_no0), sets=names(listInput_x_X_no0), keep.order=TRUE, mainbar.y.label="Number of DE genes - X chr")
